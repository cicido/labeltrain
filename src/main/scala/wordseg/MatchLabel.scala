  ...
  def main(args: Array[String]): Unit = {
    ...
    val searchDF = sparkEnv.hiveContext.sql(searchSQL).map(r => {
      ...
      val classids = r.getAs[String](1).split(",").flatMap(d => {
        ...
        toArray.sortWith(_._2 > _._2).take(5).map(r => r._1 + "|" + r._2).mkString(",")
      (imei, classids)
    })
